...
class CVFeatures(BaseModel):
    ...
    position_level: str = Field(description="The level of the job position from the CV")
    experience_years: int = Field(
        description="The number of years of experience from the CV"
    )
    technical_skills: list[str] = Field(description="The technical skills from the CV")
    experience: list[str] = Field(description="The experience from the CV")
...
class ComparisonResult(BaseModel):
    ...
    education_match: int = Field(description="The score for the education match (0-10)")
...
def extract_job_requirements(job_description: str):
    ...
    response = client.beta.chat.completions.parse(
        model=OPENAI_MODEL, messages=messages, response_format=JobRequirements
    )
...
def extract_cv_features(cv_text: str):
    ...
    response = client.beta.chat.completions.parse(
        model=OPENAI_MODEL, messages=messages, response_format=CVFeatures
    )
...
def compare_cv_with_job(cv_features: CVFeatures, job_requirements: JobRequirements):
    ...
    response = client.beta.chat.completions.parse(
        model=OPENAI_MODEL, messages=messages, response_format=ComparisonResult
    )
...
def total_match_score(comparison: ComparisonResult) -> int:
    ...
    total_score += (
        int(comparison.experience_years_match) * weights["experience_years_match"]
    )
